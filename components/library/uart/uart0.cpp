...
void UART::_sender_task(void *arg)
{
   ...
   while(1)
   {
      if(xQueueReceive(self->send_queue, &data, (TickType_t)portMAX_DELAY)) 
      {
         ...
         if (self->send_paket_sem!=NULL) xSemaphoreGive(self->send_paket_sem); 
      } //queue
   } //While
   vTaskDelete(NULL);
}
...
{  
    ...
    xSemaphoreTake(send_paket_sem, 4000 / portTICK_PERIOD_MS); //Paketin gönderilmesini bekle
                    ...
                    if (pos == -1) {
                      ...
                      if (pos>0)
                      {
                        ...
                        if (mthis->paket_decode(dtmp))
                              {
                                mthis->paket_sender = 0;//hd->sender;
                                xSemaphoreGive(mthis->callback_sem);
                                //if (self->callback!=NULL) self->callback(mthis->paket_buffer,hd->sender,TR_SERIAL, NULL);
