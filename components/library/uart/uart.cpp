                    ...
                    if (event.size==header_size) 
                      {
                        ...
                        if (event.size>=header_size)
                        {
                           ...
                           // printf("%d %d %d\n",hd->flag.paket_type, hd->receiver,mthis->get_device_id());

                            if (hd->flag.paket_type==PAKET_PING && hd->receiver==mthis->get_device_id())
                            {
                                //Ping geldiyse pong gönder
                                //printf("ping geldi %d=%d\n",mthis->pong_device,hd->sender);
                ...
                {     
                    ...
                    if (pos == -1) {
                      ...
                      if (pos>0)
                      {
                        ...
                        //printf("ping %d=%d\n",hd->sender,mthis->pong_device);

                        if(hd->sender==mthis->pong_device) mthis->ping_timer_restart();
